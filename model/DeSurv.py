...
class ODESurvSingle(nn.Module):
    ...
    def predict_surv_df(self, x_test, t_eval):
        with torch.no_grad():
            ...
            surv = pd.DataFrame(np.transpose((1 - self.predict(x_,t_).reshape((x_test.shape[0],t_eval.size))).detach().numpy()),
                                index=t_eval)
            return surv
    ...
    def compute_lik(self, x, t, k):
        # Compute loglikelihood for DeSurv
        lik_mean = -self.forward(torch.tensor(x, dtype=torch.float32)
              , torch.tensor(t, dtype=torch.float32), 
              torch.tensor(k, dtype=torch.float32)) / x.shape[0]
        
        return lik_mean.detach().item()
        
    def optimize(self, data_loader, n_epochs, logging_freq=10, data_loader_val=None,
                 max_wait=20, verbose = True):
